...
• **new Task**<TaskData, TaskResult\>(`data`)
...
• `Private` `Optional` **\_handle**: [[Handle](executor_smartq.handle.md)<[Task](executor_task.task.md)<TaskData, TaskResult\>\>, [SmartQueue](executor_smartq.smartqueue.md)<[Task](executor_task.task.md)<TaskData, TaskResult\>\>]
...
▸ `Static` **for_handle**(`handle`, `queue`, `emitter`): [Task](executor_task.task.md)<``"TaskData"``, ``"TaskResult"``\>
